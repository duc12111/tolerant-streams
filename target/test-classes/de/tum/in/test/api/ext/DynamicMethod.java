...
import de.tum.in.test.api.util.sanitization.UnexpectedExceptionError;
...
public class DynamicMethod<T> implements Checkable {
	...
	public Method getM() {
		if (m == null) {
			try {
				m = dClass.getC().getMethod(mName, DynamicClass.resolveAll(params));
	...
	public static String descArgs(Object... args) {
		return Arrays.stream(args).map(x -> x == null ? null : args.getClass().getName())
